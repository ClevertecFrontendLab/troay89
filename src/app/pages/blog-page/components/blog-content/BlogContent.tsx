...
const BlogContent = ({
    ...
    return (
        <Box px={{ base: 4, bp55: 0 }}>
            ...
            {authors && authors.favorites && authors.favorites.length !== 0 && (
                <VStack
                    p={{ base: 3, bp95: 6 }}
                    borderRadius='16px'
                    bg='lime.300'
                    mb={{ base: 8, bp95: 10 }}
                    gap={{ base: 3, bp95: 4 }}
                    data-test-id={DATA_TEST_ID.BLOGS_FAVORITES_BOX}
                >
                    <Heading
                    ...
                    >
                        {authors?.favorites?.map((author: Author, index) => (
                            <BlogCard
                                key={author._id}
                                author={author}
                                isExtraSpaceProfile={true}
                                isShowBadge={true}
                                specialFontClass={index === 0 ? 'specialFontClass' : ''}
                            />
                        ))}
                    </Grid>
                </VStack>
